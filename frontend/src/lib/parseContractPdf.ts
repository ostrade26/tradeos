...
import {
  ...
  assertSafePdfFile,
  sanitizeParsedContractStrings,
} from './pdfImportGuards'
...
async function extractPdfPages(file: File): Promise<{
  ...
  await assertSafePdfFile(file)
  const { getDocument } = await getPdfJs()
  const buffer = await file.arrayBuffer()
  // Text-extract only: no fonts/XFA/canvas — reduces attack surface of hostile PDFs.
...
export async function parseContractPdf(file: File): Promise<ParsedContractPdf> {
  ...
  const parsed = sanitizeParsedContractStrings(
    parseContractText(text, headerLines, brokerName),
  )
